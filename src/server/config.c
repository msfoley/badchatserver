...
ssize_t config_child(const char *conf_file, int fd) {
    ...
    /*void *handle = dlopen(conf_file, RTLD_NOW);
    config_func = dlsym(handle, "badchatserver_lib_config");
    ret = config_func(fd);*/
    close(fd);
    free(config_elf->buf);
    free(config_elf);
...
ssize_t config(const char *conf_file, struct config **conf) {
    ...
    *conf = malloc(len);
    ...
    char *buf = (char *) *conf;
    off_t str_endptr = (off_t) (*conf)->bind_addr;
    do {
        if (str_endptr > len) {
            log_err("Bind address string invalid\n");
            ret = -ENOBUFS;
            goto memfd_err;
        }
        str_endptr++;
    } while (*(buf + str_endptr++) != '\0');
    (*conf)->bind_addr += (uintptr_t) *conf;
